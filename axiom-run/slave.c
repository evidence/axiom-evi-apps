    ...
    for (;;) {
        ...
        if (buffer.header.command == CMD_RECV_FROM_STDIN) {
            ...
            if (info->services & EXIT_SERVICE) {
                ...
                if (res != 0) {
                    zlogmsg(LOG_WARN, LOGZ_SLAVE, "SLAVE: error sending SIGTERM signal to controlled application errno=%d '%s'!", errno, strerror(errno));
                    if (errno!=ESRCH) {
                        /* safety! */
...
/* see axiom-run.h */
int manage_slave_services(axiom_dev_t *_dev, int _services, int *_fd, pid_t _pid) {
